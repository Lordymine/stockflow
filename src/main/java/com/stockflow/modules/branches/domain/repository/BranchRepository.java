...
public interface BranchRepository extends JpaRepository<Branch, Long> {
    ...
    /**
     * Finds a branch by ID regardless of active status.
     *
     * @param id       the branch ID
     * @param tenantId the tenant ID
     * @return Optional containing the branch if found
     */
    @Query("SELECT b FROM Branch b WHERE b.id = :id AND b.tenantId = :tenantId")
    Optional<Branch> findByIdAndTenantIdIncludingInactive(@Param("id") Long id, @Param("tenantId") Long tenantId);

    /**
     * Finds active branches by tenant and a list of branch IDs.
     *
     * @param tenantId  the tenant ID
     * @param branchIds branch IDs to filter
     * @param pageable  pagination parameters
     * @return page of branches
     */
    @Query("SELECT b FROM Branch b WHERE b.tenantId = :tenantId AND b.isActive = true AND b.id IN :branchIds")
    Page<Branch> findAllByTenantIdAndIdIn(@Param("tenantId") Long tenantId,
